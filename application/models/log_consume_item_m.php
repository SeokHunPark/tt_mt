...
class Log_consume_item_m extends CI_Model
{
	...
	function get_log_list()
	{
		...
		$this->db->select('*');
		$this->db->from('drag_logdb.log_consume_item');
	...
	{
		// $sql = "select * from drag_logdb.log_consume_item where reg_date between '$begin_date' and '$end_date' order by mail_idx desc";
		// $query = $this->db->query($sql);
		// $result = $query->result();
		// return $result;
