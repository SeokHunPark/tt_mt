...
class Log_gain_item extends CI_Controller
{
	
	function __construct()
	{
		parent::__construct();
		$this->load->model('log_gain_item_m');
	...
	public function load_log()
	{
		...
		if (isset($_POST['date_search']))
		{
				...
				$end_year != "" && $end_month != "" && $end_day != "")
			{
				...
				$_log_list = $this->log_gain_item_m->get_list_with_date($user_id, $begin_date, $end_date, $size, $offset);
				
				$begin_date = $begin_year . "-" . $begin_month . "-" . $begin_day;
				$end_date = $end_year . "-" . $end_month . "-" . $end_day;
				
				$config['base_url'] = '/log/log_gain_item/load_log/date_search/' . $user_id . '/' . $begin_date . '/' . $end_date . '/';
		...
		else
		{
			if (strcmp($mode, "date_search") == 0)
			{
				...
				$_log_list = $this->log_gain_item_m->get_list_with_date($user_id, $begin_date, $end_date, $size, $offset);
				$config['base_url'] = '/log/log_gain_item/load_log/date_search/' . $user_id . '/' . $this->uri->segment(6, 0) . '/' . $this->uri->segment(7, 0) . '/';
				$config['total_rows'] = $max_rows;
				$config['per_page'] = $size;
				$config['uri_segment'] = 8;
				$this->pagination->initialize($config);
		...
		$this->load->view('/log/log_gain_item_v', $data);
	}
